...
public class FileUpDownUtil {
    ...
    //자바 웹 프로그램이 파일업로드 지원
    //1. oreilly사의 '자바서블릿프로그래밍' 도서의 예제
    //2. apache commons project - FileUpload
    // -> commons.apache.org
                ...
                while(params.hasNext()) {
                    try {
                        ...
                        if (item.isFormField()) { // 텍스트 데이터라면
                            ...
                            //업로드한 파일을 시스템에 저장할때는 파일명 + uuid로 설정
                            item.write(f);  // 지정한 경로에 파일 저장

                            String name = item.getFieldName();
                            //업로드한 파일정보를 디비에 저장할 때 원본파일명 그대로 설정
    ...
    public String proUpload2(MultipartFile mf, String uuid) {
        ...
        try {
            mf.transferTo(new File(uploadPath + nfname)); //지정한 위치로 파일 저장

            //업로드한 파일의 퍼미션을 설정, 즉, 업로드한 파일의 권한을 755(rwxr-xr-x)로 설정
