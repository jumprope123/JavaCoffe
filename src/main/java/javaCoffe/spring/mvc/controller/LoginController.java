...
//
        ...
        int resultkakao = msrv.compareID(kakaoID);

        String userid = msrv.checkUserid("uid");
        int resultour = msrv.compareID(kakaoID);
        ...
        if (uidUID == null) { //로그인 하지 않은 경우
            ...
            if (resultkakao == 0) { //우리 데이터베이스에 없는 카카오아이디다
                ...
                userid = msrv.getUserId(kakaoID);
                sess.setAttribute("UID",userid);
            }

        }else { //로그인을 한 경우 즉, 연동하기 버튼을 눌렀을때
            int cnt = msrv.inputKakao(uidUID,kakaoID); //session값과 kakaoid값을 가지고 가서 연동을 시켜준다.
...
//여기까지
    ...
    public String logout(OAuthToken oAuthToken, HttpSession sess) {
        RestTemplate rt = new RestTemplate();

        //HttpHeader 오브젝트 생성
        HttpHeaders headers = new HttpHeaders();
        headers.add("Authorization", "Bearer " + oAuthToken.getAccess_token()); // Bearer 사이에 한칸띄는게 포인트

        // HttpHeader와 Httpbody를 하나의 오브젝트에 담기
        HttpEntity<MultiValueMap<String, String>> kakaoLogoutRequest =
                new HttpEntity<>(headers);  //<<< 헤더값과 파람스값(바디)을 갖게 됨
        sess.invalidate();
        // Http 요청하기 - Post 방식으로 - 그리고 response 변수의 응답 받음. kakaoTokenRequest 담아서 받기기
        ResponseEntity<String> response = rt.exchange(
                "https://kauth.kakao.com/v1/user/logout", // 토큰발급 요청 주소
                HttpMethod.POST,
                kakaoLogoutRequest,
                String.class
        );
...
/*
    ...
    public String kakaologindel(HttpSession sess) throws IOException {
